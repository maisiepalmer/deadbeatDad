...
#if UNITY_EDITOR
        // handler for the editor inspector panel "open" button
        [UnityEditor.Callbacks.OnOpenAssetAttribute(1)]
        public static bool OnOpenAsset(int instanceID, int line)
        {
            // this technique avoids bugs where Selection.activeObject is 
            // not always updated when OnOpenAsset is called
            // see https://answers.unity.com/questions/634110/associate-my-custom-asset-with-a-custom-editorwind.html#
            // by alex_roboto - Jan 13 at 10:37 AM

            string assetPath = AssetDatabase.GetAssetPath(instanceID);
            QD_Dialogue scriptableObject = AssetDatabase.LoadAssetAtPath<QD_Dialogue>(assetPath);

            if (scriptableObject != null) // did we find the asset?
            {
                UnityEditor.EditorUtility.DisplayDialog("Whoops!",
                   "This QD_Dialogue assets is for runtime. " +
                   "You probably want to edit the QD_NodeDB " +
                   "Node Database file (in the EDITOR folder) instead", 
                                                        "Understood");
                return true;
            }
            return false; // report that we did not open
        }
#endif
