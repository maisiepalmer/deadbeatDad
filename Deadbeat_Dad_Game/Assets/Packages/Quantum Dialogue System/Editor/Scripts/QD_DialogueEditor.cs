...
namespace QuantumTek.QuantumDialogue.Editor
{
    public class QD_DialogueEditor : EditorWindow
    {
        ...
        private void OnGUI()
        {
            ...
            if (db == null)
            {
                string msg = "Please select a QD_NodeDB Node Database asset file to edit.";
                EditorGUILayout.HelpBox(msg, MessageType.Warning);
                return;
            }
            else if ( db.DataDB == null ) // phill added thiss
            {
                string msg = "The selected database does not target a QD_Dialoge runtime Data DB.";
            ...
            else
            {
                ...
                if (e.type == EventType.MouseDown && e.button == 1 && !connectingNodes)
                {
                    ...
                    {
                        selectedNode = db.Nodes[selectedIndex]; // phill moved this to happen first
                        ...
                        if (!knobClicked)
                        {
                            ...
                            menu.AddSeparator("");
                            menu.AddItem(new GUIContent("This node ID is: " + selectedNode.ID), false, null, null);
                        }
                        else
                        {
                ...
                if (connectingNodes && selectedNode != null)
                {
                    ...
                    Color curveColour = skin.customStyles[3].normal.textColor;

                    DrawNodeCurve(knobType == QD_KnobType.Output ? knobRect : mouseRect, 
                                  knobType == QD_KnobType.Output ? mouseRect : knobRect, 
                                  false,
                                  curveColour);
                    Repaint();
                }
                        ...
                        if (output.Connections.Count > 0)
                        {
                            foreach (KeyValuePair<int, List<int>> connection in output.Connections.dictionary)
                            {
                                ...
                                for (int k = 0; k < connectionCount; ++k)
                                {
                                    ...
                                    // curve colour from a speaker node
                                    if( node.Type == QD_NodeType.Speaker )
                                        curveColour = db.GetSpeakerNode(node.ID).Data.color;
                                    else if (node.Type == QD_NodeType.Message)
                                    {
                                        // curve colour from a message node (with a valid speaker)
                                        int speakerID = db.GetMessageNode(node.ID).Data.Speaker;
                                        if( speakerID != -1 )
                                            curveColour = db.GetSpeakerNode(speakerID).Data.color;
                                    }

                                    DrawNodeCurve(node.OutputKnob(j),
                                                  db.GetNode(connection.Key).InputKnob(connection.Value[k]),
                                                  true,
                                                  curveColour);
                                }
                            }
                ...
                for (int i = 0; i < nodeCount; ++i)
                {
                    ...
                    if (node.Type == QD_NodeType.Speaker)
                    {
                        ...
                        GUIStyle winStyle = GetGUIStyleForSpeakerColor(node.ID);
                        node.Window = GUI.Window(node.ID, node.Window, realNode.DrawWindow, realNode.WindowTitle, winStyle);
                    }
                    else if (node.Type == QD_NodeType.Message)
                    {
                        QD_MessageNode realNode = db.GetMessageNode(node.ID);

                        GUIStyle winStyle = GetGUIStyleForSpeakerColor(realNode.Data.Speaker);
                        node.Window = GUI.Window(node.ID, node.Window, realNode.DrawWindow, realNode.WindowTitle, winStyle);
                    }
        ...
        // phill added this
        private GUIStyle GetGUIStyleForSpeakerColor(int speakerNodeId)
        {
            GUIStyle newStyle = new GUIStyle(skin.GetStyle("window"));

            if (speakerNodeId != -1)
            {
                QD_SpeakerNode realNode = db.GetSpeakerNode(speakerNodeId);
                Texture2D newTex = MakeTexColor(2, 2, realNode.Data.color);
                newStyle.normal.background = newTex;

                // slightly different colour when window selected
                Color onColor = realNode.Data.color;
                onColor.r *= 0.5f;
                onColor.g *= 0.5f;
                onColor.b *= 0.5f;
                Texture2D newOnTex = MakeTexColor(2, 2, onColor);
                newStyle.onNormal.background = newOnTex;
            }
        ...
        // phill added this
        private Texture2D MakeTexColor(int width, int height, Color col)
        {
        ...
        // phill changed this to add colour param
        public void DrawNodeCurve(Rect start, Rect end, bool node, Color color)
        {
            //// pad with linear segments
            //float padding = 20;
            //Vector3 padStartPos = new Vector3(start.x + (node ? start.width : 0), start.y + (node ? start.height / 2 : 0), 0);
            //Vector3 padEndPos = new Vector3(padding + start.x + (node ? start.width : 0), start.y + (node ? start.height / 2 : 0), 0);
            //Handles.DrawLine(padStartPos, padEndPos);

            //// offset curve by padding amount
            //start.x += padding;
            //end.x -= padding;

            Vector3 curveStartPos = new Vector3(start.x + (node ? start.width : 0), start.y + (node ? start.height / 2 : 0), 0);
            Vector3 curveEndPos = new Vector3(end.x, end.y + (node ? end.height / 2 : 0), 0);

            float distance = Vector3.Distance(curveEndPos, curveStartPos);
            float tangentFactor = Mathf.Clamp(distance, 10, 120);

            Vector3 startTan = curveStartPos + Vector3.right * tangentFactor;
            Vector3 endTan = curveEndPos + Vector3.left * tangentFactor;
            Handles.DrawBezier(curveStartPos, curveEndPos, startTan, endTan, color, null, 3);
        }
