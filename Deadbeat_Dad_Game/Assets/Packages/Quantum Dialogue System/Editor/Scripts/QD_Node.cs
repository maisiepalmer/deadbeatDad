...
namespace QuantumTek.QuantumDialogue.Editor
{
    ...
    public class QD_Node
    {
        ...
        // phill added this 
        public bool ConnectionLoopsBack( QD_Knob outputPort )
        {
            bool doesLoopBack = false; // initially false

            // check the destination port (input "knob" on another node)
            QD_KnobDictionary connected = outputPort.Connections;
            foreach (KeyValuePair<int, List<int>> connection in connected.dictionary)
            {
                QD_Node destination = QD_DialogueEditor.db.GetNode(connection.Key);

                // does the destination port have more than one input?
                if (destination.Inputs.Count > 1)
                {
                    doesLoopBack = true;
                    break;
                }
